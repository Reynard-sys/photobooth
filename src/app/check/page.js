...
function RetakeContent() {
  ...
  // Preload edit-page assets while user reviews shots
  usePreloadImages(PRELOADS.check);
      ...
      <div className="h-dvh overflow-hidden bg-[#FDFDF5]">
        ...
        <main className="flex flex-col overflow-y-auto lg:overflow-hidden xl:overflow-hidden overscroll-none h-full justify-start w-full items-center p-10 sm:p-10 md:p-12 lg:p-24 pt-4 sm:pt-6 md:pt-0 lg:pt-30 xl:pt-20 bg-[#FDFDF5]">
          <div className="flex mt-5 md:mt-40 lg:-mt-15 xl:mt-0 mb-2 text-center">
            <Image
              src="/images/ui/retake_asset.webp"
              alt="Review your shots"
              width={240}
              height={60}
              priority
              ...
              >
                <div
                  className={`relative overflow-hidden rounded border-4 border-[#F2AEBD] max-w-[calc(23vh*16/9)] lg:max-w-[calc(20vh*16/9)] xl:max-w-[calc(23vh*16/9)] ${
                    shotParam === 3 && index === 2 ? "md:w-46 md:h-46" : ""
                  }`}
                >
                  <img className={`w-full ${shotParam === 3 && index === 2 ? "md:w-1/2" : ""}`} src={shot} alt={`Shot ${index + 1}`} />
                  <Link href={`/retake?i=${index}`} className="absolute top-2 right-2 z-20 pointer-events-auto">
                    <Image src="/images/ui/redo_asset.webp" alt="Retake" width={20} height={20} className="w-2vw" />
                  </Link>
                </div>
              </div>
            ))}
          ...
          <Link href={`/edit?s=${shotParam}`} className="relative inline-block group mt-5 mb-5">
            ...
            <div className="relative bg-[#F2DDDC] border-2 border-[#3D568F] rounded-xl py-3 lg:py-2 xl:py-5 px-30 xl:px-40 group-hover:bg-[#3D568F] group-hover:border-[#F2AEBD] transition-colors">
              <Image src="/images/ui/next_button.webp"  alt="Next Button"   width={200} height={15} priority className="pointer-events-none w-[10vw] max-w-15 lg:w-[4vw] xl:w-[3vw] h-auto group-hover:hidden" />
              <Image src="/images/ui/hover_next.webp"   alt="Next (hover)"  width={200} height={15} className="pointer-events-none w-[10vw] max-w-15 lg:w-[4vw] xl:w-[3vw] h-auto hidden group-hover:block" />
            </div>
          </Link>
        </main>
      </div>
